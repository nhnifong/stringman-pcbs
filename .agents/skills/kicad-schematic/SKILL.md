...
`scripts/` in this skill directory holds tooling for the read/render/verify loop
above; see `scripts/README.md`. `schdump.py` prints placed symbols, wires,
junctions and labels with millimetre coordinates so you can see what a net
already touches and which grid points are free. `schview.sh` renders the sheet
to PNG and crops regions by those same millimetre coordinates, which is how you
get the image input this skill demands without hunting for pixel offsets. The
README also records the sheet/library coordinate flip, how symbol rotation maps
pin 1, and the field-text angle rule -- read it before placing rotated parts.

Verify every edit two ways: diff `kicad-cli sch erc` output against the same
report generated from the pre-edit file, and read `kicad-cli sch export netlist`
to confirm new pins landed on the intended nets.
